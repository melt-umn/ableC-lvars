...
allocate datatype ProductSet with malloc;
...
allocate datatype Customer with malloc;
...
allocate datatype CustomerDatabase with malloc;
...
ProductSet* copy_products(ProductSet* p) {
  match (*p) {
    P_Empty() -> {return malloc_P_Empty();}
    P_Set(hd, tl) -> {return malloc_P_Set(hd, copy_products(tl));}
  }
}

Customer* copy_customer(Customer* c) {
  match (*c) {
    Cust(id, p) -> {return malloc_Cust(id, copy_products(p));}
  }
}

CustomerDatabase* copy_customer_database(CustomerDatabase* c) {
  match (*c) {
    CD_Empty() -> {return malloc_CD_Empty();}
    CD_Set(hd, tl) -> {return malloc_CD_Set(copy_customer(hd), 
...
ProductSet* union_prods(ProductSet* p1, ProductSet* p2) {
  match (*p1) {
    ...
    P_Set(hd1, tl1) -> {
      ...
      return malloc_P_Set(hd1, union_prods(tl1, p2));
    }
  }
}
...
Customer* union_customer(Customer* c1, Customer* c2) {
  match (*c1) {
    Cust(id1, prods1) -> {
      match (*c2) {
        Cust(_, prods2) -> {
          return malloc_Cust(id1, union_prods(prods1, prods2));
        }
      }
    }
  }
        ...
        Cust(id1, prods1) -> {
          match (*hd) {
            Cust(id2, prods2) -> {
              ...
              return malloc_CD_Set(hd, remove_customer(c, tl));
            }
          }
        }
      }
    ...
    CD_Set(hd, tl) -> {
      match (*c2) {
        ...
        _ -> {
          ...
          if (cust_in_c2 == NULL) {
            return malloc_CD_Set(copy_customer(hd), result); 
          }
          ...
          CustomerDatabase* res = malloc_CD_Set(newCust, 
...
cilk int add_data(int** store, int len) {
  ...
  int result;
  for (int i = 0; i < len; i++) {
    next = malloc_Cust(store[i][0], malloc_P_Set(store[i][1], malloc_P_Empty()));
    spawn result = cilkPut(data, malloc_CD_Set(next, malloc_CD_Empty()));
  }
  sync;
  cilk return 1;
}
...
int lookupCustomer(int id) {
  Customer* cust_to_find = malloc_Cust(id, malloc_P_Empty());
  ...
  while (1) {
    ...
    if (strcmp("exit", cmd) == 0){     
      ...
      if (strcmp("customer", cmd) == 0) {
        ...
        int numProds;
        int nextProd;
        ProductSet* result = malloc_P_Empty();
        success = fscanf(stdin, " %d:", &numProds);

        for (int i = 0; i < numProds; i++) {
          success = fscanf(stdin, " %d", &nextProd);
          result = malloc_P_Set(nextProd, result);
        }
